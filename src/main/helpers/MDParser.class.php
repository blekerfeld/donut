...
class pMDParser extends ParsedownExtra
{
    ...
    protected function internLinkLemma($input, $direct = false){
    	if($direct)
    	{
    		$result = (new pDataModel("words"))->setCondition(" WHERE id = ".p::Quote(is_numeric($input) ? $input : p::HashId($input, true)[0]))->getObjects()->fetchAll();
    		$lemma = !empty($result) ? $result[0] : null;
    		if($lemma == null)
    	...
    	if(empty($collapsed[1])){
    		$result = (new pDataModel("words"))->setCondition(" WHERE native = ".p::Quote($collapsed[0]))->getObjects()->fetchAll();
    		$lemma = !empty($result) ? $result[0] : null;
    		if($lemma == null)
    		...
    		if(p::StartsWith($collapsed[1], ':')){
    			...
    			$result = (new pDataModel("words"))->setCondition(" WHERE id = ".p::Quote(is_numeric($id) ? $input : p::HashId($id, true)[0]))->getObjects()->fetchAll();
    			$lemma = !empty($result) ? $result[0] : null;
    			if($lemma == null)
    			...
    			$result = (new pDataModel("words"))->setCondition(" WHERE native = ".p::Quote($collapsed[1]))->getObjects()->fetchAll();
    			$lemma = !empty($result) ? $result[0] : null;
    			if($lemma == null)
