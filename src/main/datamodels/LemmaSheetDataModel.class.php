...
class pLemmaSheetDataModel extends pDataModel{
	...
	public function __construct($table, $id = 0){
		...
		if($id != 0){
			...
			$this->_links['usage_notes'] = $this->loadUsageNotes();
			// Etymology
			$this->_links['etymology'] = $this->loadEtymology();
	...
	// Usage notes
	public function loadUsageNotes(){
	...
	public function loadEtymology(){
		$dM = new pDataModel('etymology');
		$dM->setCondition(" WHERE word_id = '".$this->_lemma['id']."' ");
		$dM->getObjects();
		if(array_key_exists(0, $dM->data()->fetchAll()))
			return $dM->data()->fetchAll()[0];
		else
			return false;
	}
	...
	public function updateEtymology($year, $desc){
		...
		return (new pDataModel('etymology'))->prepareForInsert(array($this->_lemma['id'], $desc, $year))->insert();
