...
if(!$word = $rs->fetchObject()){
	...
	if(!isset($donut['request']['ajaxOUT']))
		...
		elseif(isset($donut['get']['alphabetresult']))
			pOut('<div class="title"><div class="icon-box fetch"><i class="fa fa-bookmark"></i></div> Dictionary entry</div><br />'."<a class='actionbutton' href='".pUrl('?alphabet='.$donut['get']['alphabetresult'])."'><i class='fa fa-arrow-left' style='font-size: 12px!important;'></i> Back</i></a><br /><br />");
		else
	...
	foreach($modes as $mode){
		...
		foreach($numbers as $number){
			...
			foreach($submodes as $submode){
				...
				pOut('<tr><td class="singa"><em>'.$submode->name.'</em></td>');
				...
				if($show_classification_name)
					pOut('<td class="sing" style="padding: 0!important;margin:0px;">');
				else
					pOut('<td class="sing" style="width:50%">');

				if($show_classification_name)
						pOut('<table class="verbs" style="margin:0px;">');

				foreach ($gotten_classifications as $inflect_classification) {
					// The main inflection time!

					if($show_classification_name)
						pOut("<tr><td style='width:50%' class='classa'>".$inflect_classification->name."</td><td class='singb' style='width:50%'>");
					...
					if($inflections->rowCount() == 0)
						pOut("<span class='native'>".pInflect($word->id, false, $number->id, $mode->id, $submode->id, $word->classification_id, 0, '', 0, $word->subclassification_id)."</span><br />");
					else{
						while($inflection = $inflections->fetchObject()){
							pOut("<span class='native'>".pInflect($word->id, $inflection, $number->id, $mode->id, $submode->id, $word->classification_id, 0, '', 0, $word->subclassification_id)."</span><br />");
						}
