...
class pAjaxLoader extends pTemplatePiece{
	...
	public function __toString(){
		...
		$output = (($this->_loadOnClick == false AND is_array($this->_clickButton)) ? '' : "<a class='ssignore click$id ".$this->_clickButton[0]."'>".$this->_clickButton[1]."</a><br /><br />")."<div id='$id' class='$this->_wrapperclass ".(($this->_loadOnClick == true) ? 'hide ' : '')."'>".(new pIcon('fa-spinner fa-spin', 10))." ".LOADING."</div>".($this->_refresh ? "
	...
	protected function script($id){
		return 
			...
			$('.refresh$id').click(function(){
				...
				" : "")	
			."
			".(($this->_loadOnClick == false) ? "$('#$id').load('".$this->_querystring.(($this->_forceNoAjaxLink == false) ? '/ajax' : "")."');" : "$('.click$id').click(function(){
				$('#$id').slideUp().load('".$this->_querystring.(($this->_forceNoAjaxLink == false) ? '/ajax' : "")."').slideDown();
			});")."
