...
class pTablesheetTemplate extends pTemplate{
	...
	public function tablesheetForm($edit, pParadigm $paradigm = null){
		...
		foreach($this->_data->_types as $typeFetch)
			p::Out("<a class='".($typeFetch['id'] == $type['id'] ? 'active' : '')."' href='".p::Url('?grammar/tablesheet/'.$typeFetch['id'].'/new')."'>".ucfirst($typeFetch['name'])."</a>");
		p::Out("</div>");
		...
		p::Out("<br />
			...
			<a href='".p::Url('?grammar/tablesheet/'.$this->_data->id.'/new')."' class='dotted ".(!$edit ? 'active' : '')."'>".(new pIcon('asterisk'))." New table... </a>
			");
				
		// Going through the tables
		foreach($this->_data->_tables as $table)
			p::Out("<a href='".p::Url('?grammar/tablesheet/'.$this->_data->id.'/edit/'.$table['id'])."' class='".(($edit AND isset(pRegister::arg()['table_id']) AND pRegister::arg()['table_id'] == $table['id']) ? 'active' : '')."'>".ucfirst($table['name'])."</a>");
		...
		p::Out("
			...
			<div class='right'>
				<div class='btCard full'>
					...
					<div class='previewing hide loaddots'>".pMainTemplate::loadDots('center inline-block padding-left-40')."</div>
...
function updatePreview(){
	$('.previewing').slideDown();
	$('.tablePreview').slideUp().load('".p::Url('?grammar/tablesheet/'.pRegister::arg()['id'].'/preview/ajax')."', {
		'name':$('.table-name').val(),
		'headings':$('.tags.headings').val(),
		'rows':$('.tags.rows').val(),
		'columns':$('.tags.columns').val()
	}, function(){
		$('.tablePreview').slideDown();
		$('.previewing').slideUp();
	});
};";
	}
